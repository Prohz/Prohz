    ...
    public class PaymentController : Controller
    {
            ...
            string PlanId)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(subId))
                    return Json("Current subscription ID is required.");

                if (string.IsNullOrWhiteSpace(CusId))
                    return Json("Stripe customer ID is required.");

                if (string.IsNullOrWhiteSpace(PriceId))
                    return Json("Stripe price ID is required.");

                if (string.IsNullOrWhiteSpace(PlanId))
                    return Json("Plan ID is required.");
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Error starting subscription upgrade.");

                return Json(
                    "Unable to start subscription upgrade.");
            }
        }
            ...
            string PlanId)
        {
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Error starting subscription downgrade.");

                return Json(
                    "Unable to start subscription downgrade.");
            }
        }
    }
}